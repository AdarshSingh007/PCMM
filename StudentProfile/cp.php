...
// mysql_select_db("placement"); // Selecting Database from Server
	...
	if($Password && $repassword && $cur)
	{
		...
		{
			$sql = $connect->query("SELECT * FROM `placement`.`signup` WHERE `USN`='$Username'");
			if($sql->num_rows == 1)
			{
				$row = $sql->fetch_assoc();
				...
				if($cur == $dbpassword)
				{
					if($query =$connect->query("UPDATE `placement`.`signup` SET `PASSWORD` = '$Password' WHERE `signup`.`USN` = '$Username'"))
					{
